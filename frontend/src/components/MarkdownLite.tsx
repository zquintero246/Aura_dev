...
function formatTableCell(s: string): string {
  ...
  out = out.replace(/`([^`]+)`/g, '<code style="background:#f1f5f9;color:#111;padding:0 4px;border-radius:4px;">$1</code>');
  out = out.replace(/\*\*/g, '');
  return out;
}
...
function buildHtmlTableLight(header: string[], rows: string[][]): string {
  const cell = (t: string, tag: 'th' | 'td') => `<${tag} style="border:1px solid #d1d5db;padding:6px 8px;text-align:left;">${escapeHtml(t)}</${tag}>`;
  const thead = `<thead style="background:#f1f5f9;"><tr>${header.map((h) => cell(h, 'th')).join('')}</tr></thead>`;
  const tbody = `<tbody>${rows
    .map((r, i) => `<tr style="background:${i % 2 ? '#f8fafc' : 'transparent'};">${r.map((c) => cell(c, 'td')).join('')}</tr>`)
    .join('')}</tbody>`;
...
function buildHtmlTable(header: string[], rows: string[][]): string {
  const cell = (t: string, tag: 'th' | 'td') => `<${tag} style="border:1px solid rgba(255,255,255,0.15);padding:6px 8px;text-align:left;">${escapeHtml(t)}</${tag}>`;
  const thead = `<thead style="background:rgba(255,255,255,0.06);"><tr>${header
    .map((h) => cell(h, 'th'))
    .join('')}</tr></thead>`;
  const tbody = `<tbody>${rows
    .map((r, i) => `<tr style="background:${i % 2 ? 'rgba(255,255,255,0.03)' : 'transparent'};">${r
      .map((c) => cell(c, 'td'))
      .join('')}</tr>`)
    .join('')}</tbody>`;
      ...
      nodes.push(
        <Tag key={`list-${idx}-${nodes.length}`} className="mt-2 mb-2 list-outside pl-5 space-y-1">
          {listBuffer.items.map((li, j) => (
            <li key={j} className="text-[15px] leading-relaxed text-white/90" dangerouslySetInnerHTML={{ __html: formatInline(li) }} />
          ))}
        </Tag>
      );
      listBuffer = null;
...
function detectLangFromHeader(firstLine: string): LangKey | null {
  ...
  const known: Record<string, LangKey> = {
    ...
    javascript: 'javascript', js: 'js', node: 'javascript',
    typescript: 'typescript', ts: 'ts',
    bash: 'bash', sh: 'sh', shell: 'shell', zsh: 'bash',
...
function detectLangHeuristic(code: string): LangKey {
  ...
  if (/^\s*\w+\s*\{[^}]+:\s*[^;]+;/.test(s) || /\bcolor:\s*#?[0-9a-fA-F]{3,6}/.test(s)) return 'css';
...
function highlight(code: string, lang: LangKey): string {
  ...
  const span = (cls: keyof typeof color, txt: string) => `<span style="color:${color[cls]}">${txt}</span>`;

  const applyCommon = (text: string, options: { lineComments?: RegExp; blockComments?: RegExp } = {}) => {
    // Strings
    ...
    if (options.blockComments)
      text = text.replace(options.blockComments, (m) => span('com', m));
    return text;
  };
  ...
  switch (lang) {
    case 'python': {
      ...
      src = src.replace(/\b(def|class|return|if|elif|else|for|while|try|except|with|as|from|import|pass|lambda|yield|raise|True|False|None|async|await|print)\b/g, (m) => span('kw', m));
    ...
    case 'ts': {
      ...
      src = src.replace(/\b(const|let|var|function|return|if|else|for|while|import|from|export|class|new|try|catch|finally|true|false|null|undefined|async|await|type|interface)\b/g, (m) => span('kw', m));
      src = src.replace(/\b([a-zA-Z_$][\w$]*)\s*\(/g, (m, g1) => span('fn', g1) + '(');
      break;
    }
    case 'bash':
    ...
    case 'shell': {
      src = applyCommon(src, { lineComments: /#.*/g });
      src = src.replace(/\b(if|then|fi|for|in|do|done|case|esac|function|elif|else|echo|exit)\b/g, (m) => span('kw', m));
      break;
    }
    case 'powershell': {
      src = applyCommon(src, { lineComments: /#.*/g });
      src = src.replace(/\b(function|param|if|elseif|else|for|foreach|return|try|catch|finally)\b/gi, (m) => span('kw', m));
    ...
    case 'php': {
      ...
      src = src.replace(/\$[a-zA-Z_][\w]*/g, (m) => span('prop', m));
      src = src.replace(/\b(function|class|public|protected|private|static|use|namespace|if|else|elseif|return|echo|try|catch)\b/g, (m) => span('kw', m));
      src = src.replace(/\b([a-zA-Z_][\w]*)\s*\(/g, (m, g1) => span('fn', g1) + '(');
      break;
    }
    case 'json': {
    ...
    case 'xml': {
      // tags/attrs
      src = src.replace(/(&lt;\/?)([a-zA-Z][a-zA-Z0-9:-]*)([^&]*?)(\/?&gt;)/g, (_, open, tag, attrs, close) => {
        const attrColored = attrs.replace(/([a-zA-Z_:][-a-zA-Z0-9_:.]*)(=)(&quot;[^&]*&quot;|[^\s>&]+)/g, (m, a, eq, v) => `${span('prop', a)}${eq}${span('str', v)}`);
    ...
    case 'css': {
      ...
      src = src.replace(/([a-zA-Z0-9_\-\.\#]+)(\s*\{[\s\S]*?\})/g, (m, sel, block) => `${span('tag', sel)}${block}`);
      src = src.replace(/([a-zA-Z-]+)(\s*:\s*)([^;]+)(;?)/g, (m, p, sep, v, end) => `${span('prop', p)}${sep}${span('str', v)}${end}`);
      break;
    }
    case 'sql': {
      src = applyCommon(src, { lineComments: /--.*$/gm, blockComments: /\/\*[\s\S]*?\*\//g });
      src = src.replace(/\b(SELECT|FROM|WHERE|JOIN|LEFT|RIGHT|INNER|OUTER|ON|GROUP BY|ORDER BY|INSERT|INTO|VALUES|UPDATE|SET|DELETE|CREATE|TABLE|PRIMARY|KEY|NOT NULL|AND|OR|AS|LIMIT|OFFSET)\b/gi, (m) => span('kw', m));
      break;
    }
    case 'yaml':
    case 'yml': {
  ...
  return (
    <div className="mt-2 mb-3 overflow-hidden rounded-md bg-black/30 ring-1 ring-white/10">
      <div className="flex items-center justify-between px-3 py-1.5 text-[12px] text-white/70 bg-white/5 border-b border-white/10">
        ...
        <button onClick={onCopy} className="px-2 py-0.5 rounded bg-white/10 hover:bg-white/15 ring-1 ring-white/10">
          {copied ? 'Copiado' : 'Copiar'}
        </button>
      </div>
